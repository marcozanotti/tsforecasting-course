...
```{r}
ensemble_results <- eventReactive(
  ...
  valueExpr = {
    map(
      ...
      generate_forecast(
        data = data_cleaned(), method = input$ens_method, n_future = input$ens_n_future,
        n_assess = input$ens_n_assess, assess_type = input$ens_assess_type,
        ensemble_method = input$ens_type
      )
  }
)

stacking_results <- eventReactive(
  ...
  valueExpr = {
    map(
      ...
      generate_forecast(
        data = data_cleaned(), method = input$ens_method, n_future = input$ens_n_future,
        n_assess = input$ens_n_assess, assess_type = input$ens_assess_type,
        ensemble_method = input$stk_type
      )
  }
)

output$plot_ens_test_forecast <- renderPlotly({
  if (input$ens_apply_forecast) {
    ensemble_results()$test_forecast |> 
      plot_modeltime_forecast(.legend_show = TRUE, .title = FALSE, .conf_interval_fill = "lightblue")
  } else if (input$stk_apply_forecast) {
    stacking_results()$test_forecast |> 
      plot_modeltime_forecast(.legend_show = TRUE, .title = FALSE, .conf_interval_fill = "lightblue")
  }
})
...
output$plot_ens_oos_forecast <- renderPlotly({
  if (input$ens_apply_forecast) {
    ensemble_results()$oos_forecast |> 
      plot_modeltime_forecast(.legend_show = TRUE, .title = FALSE, .conf_interval_fill = "lightblue")
  } else if (input$stk_apply_forecast) {
    stacking_results()$oos_forecast |> 
      plot_modeltime_forecast(.legend_show = TRUE, .title = FALSE, .conf_interval_fill = "lightblue")
  }
})
...
output$ens_model_summary <- renderPrint({
  if (input$ens_apply_forecast) {
    ensemble_results()$fit
  } else if (input$stk_apply_forecast) {
    stacking_results()$fit
  }
})
```
