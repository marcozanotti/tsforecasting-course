...
methods_params_cl <- methods_params |> map( ~ stringr::str_to_title(stringr::str_replace_all(., "_", " ")))
```
...
```{r}
# data_table <- reactive({
#   if (input$table_type == "preview") { data_selected() }
# })
# output$data_table <- renderDT({ data_table() })
# 
# data_str <- reactive({
#   if (input$table_type == "str") { str(data_selected()) }
# })
# output$data_str <- renderPrint({ data_str() })
# 
# data_summ <- reactive({
#   if (input$table_type == "summary") { summary(data_selected()) }
# })
# output$data_summ <- renderPrint({ data_summ() })
# 
# observe({
#   req(input$table_type)
#   switch(
#     input$table_type,
#     data_table = { renderUI({ DTOutput(outputId = "data_table") }) },
#     data_str = { renderUI({ verbatimTextOutput(outputId = "data_str") }) },
#     data_summ = { renderUI({ verbatimTextOutput(outputId = "data_summ") }) }
#   )
# })
  
# https://stackoverflow.com/questions/59069454/r-shiny-switch-between-table-and-plot-in-renderui
# 
# output$data_table <- renderDT({ data_selected() })
# output$data_str <- renderPrint({ str(data_selected()) })
# output$data_summ <- renderPrint({ summary(data_selected()) })
# 
# reactive({
#   req(input$table_type)
...
#   } else if (input$table_type == "summary") {
...
#   switch(
#     input$table_type,
#     preview = DTOutput(outputId = "data_table"),
#     str = verbatimTextOutput(outputId = "data_str"),
#     summ = verbatimTextOutput(outputId = "data_summ")
#   )
# })
```
...
selectInput(
  inputId = "method", label = h3("Forecast Algorithm"), 
  choices = methods, selected = methods[1]
)
...
selectInput(
  inputId = "tune_method", label = h3("Forecast Algorithm"), 
  choices = ml_methods, selected = ml_methods[2]
)
...
pickerInput(
  inputId = "comp_method", label = h3("Forecast Algorithms"), 
  choices = methods, selected = methods[c(2, 3)], multiple = TRUE,
  options =  list("actions-box" = TRUE)
)
...
observeEvent(
  ...
  handlerExpr = {
    ...
    updatePickerInput(session = session, inputId = "comp_method", selected = methods[c(2, 3)])
...
observeEvent(
  ...
  handlerExpr = {
    ...
    updatePickerInput(session = session, inputId = "ens_method", selected = methods[c(2, 3)])
...
selectInput(
  inputId = "scn_method", label = h3("Forecast Algorithm"), 
  choices = methods, selected = methods[2]
)
...
observeEvent(
  ...
  handlerExpr = {
    ...
    updateSelectInput(session = session, inputId = "scn_method", selected = ml_methods[2])
    # shinyjs::delay(ms = 300, expr = {shinyjs::click(id = "tune_apply_forecast")})
  }
)
```
