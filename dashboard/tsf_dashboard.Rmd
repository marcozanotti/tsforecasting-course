  ...
  data_selected() |>
      datatable(
        options = list(
          ...
          info = FALSE, paging = TRUE, scrollCollapse = TRUE, scrollY = 560 # 250
        )
      )
})
...
forecast_results <- eventReactive(
  ...
  valueExpr = {
    map(
      input$method, 
      ~ fit_model(
        data = data_cleaned(), method = ., params = input, 
...
compare_results <- eventReactive(
  ...
  valueExpr = {
    map(
      ...
      generate_forecast(
        data = data_cleaned(), method = input$comp_method, n_future = input$comp_n_future,
...
output$plot_comp_oos_forecast <- renderPlotly({
  compare_results()$oos_forecast |> 
    plot_modeltime_forecast(.legend_show = TRUE, .title = FALSE, .conf_interval_fill = "lightblue")
})

output$comp_accuracy_table <- renderDT({
  compare_results()$accuracy |> 
...
### Evaluation Metrics {data-width=600 .no-padding}

```{r}
DTOutput(outputId = "comp_accuracy_table")
```

### Algorithm Summary {data-width=400 .no-padding}
...
tune_results <- eventReactive(
  ...
  valueExpr = {
    map(
      ...
      generate_forecast(
        data = data_cleaned(), method = input$tune_method, n_future = input$tune_n_future,
        n_assess = input$tune_n_assess, assess_type = input$tune_assess_type
      )
  }
)
...
output$plot_tune_oos_forecast <- renderPlotly({
  tune_results()$oos_forecast |> 
    plot_modeltime_forecast(.legend_show = FALSE, .title = FALSE, .conf_interval_fill = "lightblue")
})

output$tune_accuracy_table <- renderDT({
  tune_results()$accuracy |>
    format_accuracy(single_method = TRUE) |>
    datatable(
      options = list(
        ordering = FALSE, pageLength = 20, lengthChange = FALSE, searching = FALSE,
    ...
    react_val$combine_results <-
      map(
        ...
        ~ fit_model(
          ...
          method = .,
          params = input,
          n_assess = input$ens_n_assess,
          assess_type = input$ens_assess_type,
          seed = 1992
        )
      ) |>
      generate_forecast(
        data = data_cleaned(),
        method = input$ens_method,
        n_future = input$ens_n_future,
        n_assess = input$ens_n_assess,
        assess_type = input$ens_assess_type,
        ensemble_methods = input$ens_type,
...
output$plot_ens_test_forecast <- renderPlotly({
  req(react_val$combine_results$test_forecast)
  react_val$combine_results$test_forecast |>
    plot_modeltime_forecast(.legend_show = TRUE, .title = FALSE, .conf_interval_fill = "lightblue")
})


output$plot_ens_oos_forecast <- renderPlotly({
  req(react_val$combine_results$oos_forecast)
  react_val$combine_results$oos_forecast |> 
    plot_modeltime_forecast(.legend_show = TRUE, .title = FALSE, .conf_interval_fill = "lightblue")
})

output$ens_accuracy_table <- renderDT({
  req(react_val$combine_results$accuracy)
  react_val$combine_results$accuracy |>
    format_accuracy(single_method = FALSE) |>
    filter(Type == "Test") |>
    datatable(
      options = list(
...
br()
sliderInput(
  inputId = "scn_conf_lvl", label = "Confidence Level", 
  min = 0.05, max = 0.99, value = 0.9, step = 0.05
)
br()
selectInput(
  inputId = "scn_agg_fun", label = "Aggregation Function", 
  choices = c("sum", "mean", "median"), selected = "sum"
)
br()
numericInput(
  inputId = "scn_adj", label = "Business Adjustment (%)", 
...
observeEvent(
  ...
  handlerExpr = {
    ...
    updateSelectInput(session = session, inputId = "scn_agg_fun", selected = "sum")
    updateNumericInput(session = session, inputId = "scn_adj", value = 0)
    # shinyjs::delay(ms = 300, expr = {shinyjs::click(id = "tune_apply_forecast")})
  }
)
```
...
scenario_results <- eventReactive(
  ...
  valueExpr = {
    map(
      ...
      generate_forecast(
        data = data_cleaned(), method = input$scn_method, n_future = input$scn_n_future,
...
```{r}
# DTOutput(outputId = "scn_delta_table")
```
